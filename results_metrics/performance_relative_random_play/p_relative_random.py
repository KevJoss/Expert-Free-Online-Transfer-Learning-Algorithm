...
f42 = s42[s42.Episode >= 1600].Reward.values
f7  = s7 [s7.Episode  >= 1600].Reward.values
...
for n, x in [('base', R), ('s42', f42), ('s7', f7)]:
    print(n, len(x), round(x.mean(),1), np.median(x), np.percentile(x,25), np.percentile(x,75))

print()
print(cliff(f42, R), cliff(f7, R))

print()

rng = np.random.default_rng(1)  # Generador aleatorio con semilla fija
def ci_dif(a, b, B=20000, seed=1):
...
print('dif 42:', round(f42.mean()-R.mean(), 2), ci_dif(f42, R).round(1))
print('dif  7:', round(f7.mean() -R.mean(), 2), ci_dif(f7,  R).round(1))