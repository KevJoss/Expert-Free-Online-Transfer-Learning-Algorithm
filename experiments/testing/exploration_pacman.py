...
agents_directory = os.path.join(grandparent_directory, 'agents')
...
def run_pacman_simulation(sim_id, seed, models_dir, param_file):
    ...
    for episode in range(NUM_EPISODES):
        ...
        # --- Every 100 episodes: console progress line --------------------
        if (episode + 1) % 100 == 0:
            _stdout_interceptor.flush_and_log()
...
for sim_id in range(NUM_SIMULATIONS):
    result = run_pacman_simulation(sim_id=sim_id, seed=42, models_dir=MODELS_DIR, param_file=CONFIG_FILE)
