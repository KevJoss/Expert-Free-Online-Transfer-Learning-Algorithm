...
# Load the JSON configuration file "configs/cart-pole_params.json"
CONFIG_FILE = "configs/cart-pole_params.json"
...
LOG_FILE    = os.path.join(_SCRIPT_DIR, "seed42_cart-pole.log")
...
def run_cartpole_simulation(sim_id, seed, models_dir, param_file):
    """
    Runs one complete EFo-TL training session for the CartPole-v1 environment.
    CartPole states are already normalized, so no normalization is applied.
    """

    set_seed(seed)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"\n{'='*70}")
    print(f"  RUNNING EF-OnTL algorithm (CartPole-v1)")
    ...
    # Parse hidden_dim parameter for agent
   ...
   # --- Logging / stats accumulators ------------------------------------
    ...
    for episode in range(NUM_EPISODES):
        for data in agents_data:
            ...
            # NOTE: CartPole states are already normalized — no division applied
            done = False
            ep_reward = 0
...
for sim_id in range(NUM_SIMULATIONS):
    result = run_cartpole_simulation(sim_id=sim_id, seed=42, models_dir=MODELS_DIR, param_file=CONFIG_FILE)
